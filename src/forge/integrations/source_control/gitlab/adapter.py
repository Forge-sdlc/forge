...
class GitLabAdapter:
    ...
    @staticmethod
    def _cache_put(
        cache: OrderedDict, key, value, *, max_size: int = _MAX_FORK_PROJECT_CACHE_ENTRIES
    ) -> None:
        cache[key] = value
    ...
    @translate_provider_errors("GitLab")
    async def get_change_request_comments(
        self, repo_ref: RepositoryRef, identity: ChangeRequestIdentity
    ) -> list[ReviewComment]:
        """Return all discussion notes for effect idempotency checks."""
        discussions = await self._get_client().get_discussions(
            repo_ref.namespace, _require_native_id(identity)
        )
        return [
            self._map_note_response(note)
